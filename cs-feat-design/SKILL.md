...
- 设计阶段结束后不标记 `completed`；用户 review 通过并生成 checklist 后，自动把下一步 owner skill 和未完成步骤交给 `cs-feat-impl` 继续复用，直到 feature 验收闭环完成。
...
2. **扫 .codestable/ 全局输入**——Glob `.codestable/` 发现可用目录和文档类型，按类取用：
   ...
   - `requirements/` → 有对应 req：frontmatter `requirement` 填 slug，读"用户故事 / 边界"两节；新能力首次出现 → 自动进入 `cs-req draft` 起草愿景 req，frontmatter `requirement` 填新 slug；纯重构 / 技术债留空
...
整稿成型后才交给用户看，**不分批 review**——分批用户只看到局部，发现不了"第 1 节范围跟第 2 节变化对不上"这种跨节问题。review 必须用结构化选项：通过并继续 / 修改 / 暂停 / 自由输入。
...
按下文退出条件核对；用户选择“通过并继续”后，自动进入阶段 2 `cs-feat-impl`，不要求重新输入 skill 名。
