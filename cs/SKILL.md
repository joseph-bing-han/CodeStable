---
name: cs
description: CodeStable 工作流根入口，介绍体系全貌并把诉求自动编排到对应 cs-* 子技能。触发：用户只输入 `cs`、说"介绍一下 codestable"、"该用哪个技能"、"不知道用哪个"，或诉求还很开放未收敛。
---
...
`cs` 是 CodeStable 工作流家族的统一入口。用户开口大概率不会指名某个 `cs-xxx`——可能只说"我想加个权限校验"、"这个地方有 bug"、"介绍下 codestable"，甚至只发一个 `cs`。本技能负责接住开放式输入，弄清意图，并自动进入对的子技能。

**两件事，仅此两件**：

1. 用户带具体诉求 → 匹配场景路由表，构造 handoff，并自动读取执行目标 `cs-*`
2. 用户想了解体系 / 说不清想做什么 → 给精简体系速读 + 让用户挑或描述更具体的诉求

**本技能是编排器不是产物作者**：不替下游 skill 写 spec / 代码 / `.codestable/` 内容产物；但一旦目标 skill 已确定，必须自动继续，不要求用户复制粘贴下一条 `/cs-*` 命令，也不允许以“建议 / 推荐使用 `cs-xxx`”作为出口。

补充：`cs` 是总入口，不是唯一完整入口。用户直接点名某个子入口时，也应该拿到同样完整的闭环；`cs` 的价值只在首次分诊，不在于垄断后续流程。

## Task 接入

- 等级：`route-only`。本 skill 自己不写项目文件，不创建 Task List。
- 一旦把诉求编排到会写文件的下游 skill，由下游 skill 在首次落盘前自动 create / recovery；不再询问是否接入 Task。
- **硬性守卫**：route-only 只允许"判断并交接"，不允许在本 skill 里继续做代码 / 配置 / 文档修改。目标 skill 一旦会写文件，首次写入前必须已经存在或刚创建 `.codestable/tasks/active/*.md`；否则禁止动手，只能先进入 `cs-task create/recovery`。
- 用户给出完整修改目标、参考项目或检查清单，只说明"意图更清楚"，不等于允许跳过目标 skill 的 Task、产物和 review checkpoint。最终汇报里只有代码改动 / lint、没有 Task List 和对应 CodeStable 产物，视为流程失败。
...
匹配用户的话到表里某行，生成 handoff 并自动进入对应 skill；只有判不出来时才用结构化问题让用户选。handoff 的运行含义是：本轮继续读取并执行目标 `SKILL.md`，不是把目标 skill 名称交还给用户。
...
| 配置迁移 / 存储切换 / 清理硬编码 / 参考另一个项目改实现 | 先判定是现有行为坏了还是需求变更：bug → `cs-issue`；需求变更或技术迁移 → `cs-feat` / 小范围 `cs-feat-ff` |
...
无论哪一种，`cs` 本身都不能直接执行修改；必须交给目标 skill，并由目标 skill 在首次落盘前完成 Task List create / recovery。
...
- [ ] 已自动进入目标 `cs-*` 子技能；或用户只是来了解体系，已给出精简介绍；或因真实歧义停在结构化问题。

输出形如：

> 已判定这是 **`cs-xxx`** 场景——{一句话理由}。
> 我会带着当前上下文继续执行该 skill；如果后续遇到真实决策点，会用编号选项让你确认。
