...
**本技能是编排器不是产物作者**：不替下游 skill 写 spec / 代码 / `.codestable/` 内容产物；但一旦目标 skill 已确定，必须自动继续，不要求用户复制粘贴下一条 `/cs-*` 命令。

## Task 接入

- 等级：`route-only`。本 skill 自己不写项目文件，不创建 Task List。
- 一旦把诉求编排到会写文件的下游 skill，由下游 skill 按 `auto` 或 `ask` 策略负责创建 / 复用 Task List。
...
匹配用户的话到表里某行，生成 handoff 并自动进入对应 skill；只有判不出来时才用结构化问题让用户选。
...
| 当前任务 / 继续任务 / 任务列表 / 历史任务 / 归档任务 | `cs-task` |
...
- **不替下游产出 `.codestable/` 内容产物**——这些是子技能的事，但可以自动交接执行
