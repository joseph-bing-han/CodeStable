...
- 等级：`auto`。落盘 `{slug}-qa.md` 前必须复用 / 创建当前 feature 的 Task List（`cs-task`），无 Task 不动手。
- QA 报告落盘、verdict 定稿后，先更新 Task List 步骤状态与文档索引再继续。
- 不标 `completed`：只把任务推进到下一阶段（passed → `cs-feat-accept`；failed / blocked → `cs-feat-impl` qa-fix），feature Task 由末端 `cs-feat-accept` 收口。
