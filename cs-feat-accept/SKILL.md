---
name: cs-feat-accept
description: feature 流程阶段 3——验收闭环：对照 design 核实现 + 回写 architecture / requirement / roadmap，产出 {slug}-acceptance.md，并把最终质量门禁交给 `cs-code-review`。触发：用户说"功能写完了验收一下"、"做最后检查"、"准备 merge"、"出验收报告"。前置依赖 cs-feat-impl 完成。
---
...
- 验收报告完成、关联回写完成、用户终审通过后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；只有 code review 通过后，才将该 feature Task List 标记 `completed`；归档由 `cs-task` 执行并清理 active 残留。
...
- [ ] `requirement` 空 + 新增了用户可感能力 → 自动进入 `cs-req` **backfill** 直接落 `status: current`
- [ ] `requirement` 指向 draft req → 自动进入 `cs-req` **update**：`draft` → `current`，按实际实现刷新用户故事 / 边界，**保留原始愿景**（愿景不被覆盖，只在文末加变更日志记录本次改动）
- [ ] `requirement` 指向 current req 且本次改了边界 / 用户故事 / pitch → 自动进入 `cs-req` **update** 刷新
...
告诉用户："验收报告已就绪，架构文档已归并。下一步进入 `cs-code-review` 做最终质量门禁；review 通过后 cs-feat 才算闭环。"
...
5. 第 8 节有 attention.md 候选 → 逐条问"候选 X 加到 attention.md 吗？" 用户明确同意 → 自动进入 `cs-note` 走分节归类 / 查重 / 软上限检查（不在 accept 里手写，避免和 cs-note 各搞一套口径）；**一次一条**
6. review 通过后如需 scoped-commit，由 `cs-code-review` 统一按 `shared-conventions.md` 第 4 节发起确认

收尾提交规则看 `shared-conventions.md` 第 4 节。提交范围：功能代码 + 方案 doc + 验收报告 + `{slug}-code-review.md` + 本次实际更新的架构 doc / req doc / roadmap items.yaml + 主文档；是否执行由 `cs-code-review` 统一确认。
