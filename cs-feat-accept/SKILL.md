...
- 验收报告完成、关联回写完成、用户终审通过后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；只有 code review 通过后，才将该 feature Task List 标记 `completed` 并立即进入 `cs-task archive`；退出条件以 active 中无同名残留为准。
