...
- 等级：`auto`。本技能编排整条大需求 goal，对应该 roadmap 流程的 Task List（`cs-task`）。
- 首次落盘（roadmap / design / goal 执行包等产物）前复用或创建当前流程 Task List，无 Task 不动手；产物落盘先更新 Task List 再继续；编排阶段非末端不标 `completed`。
- 子 feature 的实现由 `/goal` 会话逐个跑 `cs-feat-impl` 等实现技能，gate 与 code-review 由那些技能自身负责，本技能不照搬 gate 命令。
