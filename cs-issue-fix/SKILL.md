---
name: cs-issue-fix
description: issue 流程阶段 3——按已确认根因和方案定点修复、验证、写 {slug}-fix-note.md 落档，并把最终质量门禁交给 `cs-code-review`。两个入口：标准路径从 analyze 来，快速通道从 report 直接来。触发：用户说"开始修 bug"、"按分析修"、"动手改代码"。只动方案声明的文件，不顺手优化。
---
...
- 等级：`auto`（重要变更）。本 skill 必须复用当前 issue 的 Task List；在首次修改代码、记录验证结果或写 `{slug}-fix-note.md` 前保持同步。
- 每完成一处修复、每次验证清单状态变化、或补记顺手发现 / 日志调试结论后，先更新 Task List 的步骤状态和 CodeStable 文档索引，再继续流程。
- 修复验证完成、fix-note 落盘、用户明确确认 bug 已修复后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；只有 code review 通过后，才将该 issue Task List 标记 `completed` 并**在同一轮执行 `cs-task archive` 的实际归档动作**；退出条件以 active 中无同名残留为准。不能只提示“下一步归档”，也不能停在“是否提交 commit”的询问前让用户手动发起归档。
- **无 Task 不修复**：启动时先定位当前 issue 对应的 Task spine，必须同时检查 `.codestable/tasks/active/*.md` 与 `.codestable/tasks/archived/YYYY-MM-DD-*.md`。active 找不到但 archived 中已有同 slug 且 `status: archived` 时，说明任务已经闭环，禁止补建新的 active Task；只有 active 与 archived 都不存在时，才补建 / 复用 Task List，并把 `owner_skill` 设为 `cs-issue-fix`。补建前禁止修改代码、配置、测试或 `.env`。
- 快速通道从 report 直接来时也必须带着 Task List 和 issue 目录进入；如果上游只给了口头根因，没有 Task / issue 目录，先回到 `cs-issue-report` 补齐快速通道入口。
- **无 review 不闭环**：fix-note 落盘后，本阶段不能以“下一步应进入 `cs-code-review`”收尾；当前 agent 必须立即读取并执行 `cs-code-review`。如果 review 前置不满足，只能把 Task 留在 active 并记录阻塞原因。
...
## 修复门禁（Task + review evidence）

进入修复前先确认三件事：issue 目录存在、根因与方案已被用户确认、active Task 已登记本 issue。任一项缺失就退回对应阶段补齐，不开始改代码。

修复完成后不能只汇报“测试已过”。必须先写 `{slug}-fix-note.md`、更新 Task 账本，再立即进入 `cs-code-review` 产出 `{slug}-code-review.md`；review 通过前 Task 不能 completed。
...
1. **确认 Task 与 issue 目录已存在**——没有就回 `cs-issue-report` 补齐，不在 fix 阶段裸建裸改
2. **明确陈述根因**："`{文件}:{行号}` 的 {具体代码} 存在 {问题描述}"，让用户确认根因判断准确
3. **给修复方案**——改哪里、怎么改（一两句话，不写完整分析文档）
4. **等用户明确说"对，就这样改"才动手**——不允许"我觉得对，直接改了"
5. 读 `.codestable/attention.md`
...
- [ ] 没有范围外改动（或已和用户确认）
- [ ] 用户明确确认修复完成
- [ ] 已实际执行 `cs-code-review`，或因明确阻塞记录在 Task 中；不能只提示下一步
...
- **提交范围**：修复代码 + `{slug}-fix-note.md` + `{slug}-code-review.md` + 本次一并更新的 report / analysis
- 修复闭环后自动执行 `cs-code-review` 做最终质量门禁；review 通过后再问是否需要 commit

---

## 退出后

告诉用户："issue 修复验证完成，report + analysis + fix-note 已存档。" 随后立即进入 `cs-code-review`；不要把它作为给用户的下一步建议。
