...
- 等级：`auto`。会写 `.codestable/issues` 产物，落盘前对齐 Task List。
- 首次落盘前复用/创建当前 issue 的 Task List，无 Task 不动手；阶段产物落盘先更新 Task List 再继续。
- 收口：修复验证通过、code-review 通过后标 `completed` 并转 `cs-task` archive。
