...
- 修复验证完成、fix-note 落盘、用户明确确认 bug 已修复后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；只有 code review 通过后，才将该 issue Task List 标记 `completed`；归档由 `cs-task` 执行并清理 active 残留。
...
- 修复闭环后告诉用户"修复验证已完成，`{slug}-fix-note.md` 已落盘，下一步进入 `cs-code-review` 做最终质量门禁"；review 通过后再问是否需要 commit

---

## 退出后

告诉用户："issue 修复验证完成，report + analysis + fix-note 已存档。下一步进入 `cs-code-review`；review 通过后 issue 工作流才闭环。"
...
4. review 通过后如需提交，由 `cs-code-review` 统一按收尾提交规则发起确认
