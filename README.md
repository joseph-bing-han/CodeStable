...
| 流程 | 关键技能链 | 说明 |
|------|------------|------|
| **特性引入** | `cs-feat` → `cs-feat-design` → `cs-feat-impl` → `cs-feat-accept` → `cs-code-review` | 想清楚 → 综合架构设计 → 逐步编码 → 验收测试 → 独立代码评审 |
| **问题修改** | `cs-issue-report` → `cs-issue-analyze` → `cs-issue-fix` → `cs-code-review` | 跟 AI 说哪里有问题 → 让 AI 分析根因 → 让 AI 定点修复 → 合并前审当前 diff |
...
<tr><td><code>cs-feat-accept</code></td><td>逐层对照 design 核对实现，完成验收后把最终质量门禁交给 <code>cs-code-review</code></td></tr>
<tr><td><code>cs-feat-ff</code></td><td>超轻量通道：不写 design、不分阶段，快速实现后仍进入 <code>cs-code-review</code></td></tr>
...
<tr><td><code>cs-issue-fix</code></td><td>定点修复 + 验证 + 写 fix-note，之后进入 <code>cs-code-review</code></td></tr>
<tr><td rowspan="2"><b>重构流程</b></td><td><code>cs-refactor</code></td><td>(beta) 重构主流程；apply 后进入 <code>cs-code-review</code></td></tr>
<tr><td><code>cs-refactor-ff</code></td><td>(beta) 轻量重构通道；快速改完后仍进入 <code>cs-code-review</code></td></tr>
<tr><td><b>质量门禁</b></td><td><code>cs-code-review</code></td><td>feature / issue / refactor 完成后、scoped-commit / PR / merge 前的独立代码评审</td></tr>
...
───────────────────────────────────────────────────────────────────────
  ...
  ▸ 事件：新增能力                                          ┌──────────┐
       cs-feat-design ──▶ cs-feat-impl ──▶ cs-feat-accept ─▶ cs-code-review
                                                            │ features │
       cs-feat-ff     ──(轻量直通车，跳过 design/accept)────▶ cs-code-review
                                                            │ /YYYY-…/ │
                                                            └──────────┘
...
`/cs-onboard` 跑完后，会在项目根生成 `.codestable/` 目录，作为 CodeStable 内部工作流工件的聚合根；对外指南和 API 参考默认仍在 `docs/`。完整目录结构见 [docs/runtime-structure.md](docs/runtime-structure.md)。
