...
| **任务** | tasks | 跨流程的任务账本（active / archived），中断可恢复、完成可归档 |
...
`cs-code-review` 是各执行流末端、commit 前的横切质量门禁；`cs-task` 为所有落盘流程记账。阶段或里程碑收尾时，用 `cs-docs-neat` 整理 `.codestable/`、README/docs、`CLAUDE.md` / `AGENTS.md` 和 agent 记忆，避免文档与代码脱节。
...
`/cs-onboard` 跑完后，会在你的项目根下生成 `.codestable/`，作为 requirements、architecture、roadmap、goals、features、issues、refactors、audits、tasks、compound、tools、hooks 和 reference 的聚合根。
