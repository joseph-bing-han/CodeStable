...
- 等级：`auto`（重要变更）。本 skill 开始实现前必须复用当前 feature 的 Task List；如果前序阶段还没创建，则此处补建后再动代码。
- 每完成一个 checklist step、每次新增代码相关验证证据、或发现需要追加执行步骤时，先更新 Task List 的步骤状态、`owner_skill` 和 CodeStable 文档索引，再继续实现。
- 实现阶段结束不标记 `completed`；只把任务推进到 `cs-feat-accept`，由验收阶段完成整条 feature Task 的收口。
...
所有步骤完成后用下面模板汇报，并用结构化选项让用户 review：通过并进入验收 / 需要修改 / 暂停 / 自由输入。
...
告诉用户："所有步骤完成，方案 doc 已同步。下一步阶段 3 验收闭环。" 用户选择“通过并进入验收”后，自动进入 `cs-feat-accept`。

别在用户 review 通过前开始写验收报告——验收需要独立的 checklist 节奏，提前进入会让把关失效。
...
- 用户 review 还没通过就自己进入验收阶段；或 review 通过后还要求用户手动触发 `cs-feat-accept`
- 关键场景清单一条都没落证据
