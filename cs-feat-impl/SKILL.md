...
- 等级：`auto`（重要变更）。开始实现前必须复用当前 feature 的 Task List（`cs-task`）；前序阶段还没创建就此处补建后再动代码。
- 每完成一个 checklist step、每次新增验证证据、或发现需要追加执行步骤时，先更新 Task List 的步骤状态、`owner_skill` 和 CodeStable 文档索引，再继续实现。
- 实现阶段结束不标记 `completed`；只把任务推进到 `cs-code-review`，由后续评审 / QA / 验收阶段收口整条 feature Task。
