...
## 实现门禁（预检 + review evidence）

进入实现前先确认三件事：design 已 approved、checklist 存在且当前步骤可定位、active Task 已登记本 feature。任一项缺失就退回对应阶段补齐，不开始改代码。

实现完成后不能只汇报“测试已过”。必须先更新 checklist step 状态和 Task 账本，再进入 `cs-code-review` 产出 `{slug}-review.md`；review 通过前 Task 不能 completed。
