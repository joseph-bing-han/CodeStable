...
- 等级：`ask`（次要变更）。本 skill 在首次写 `.codestable/compound/YYYY-MM-DD-trick-{slug}.md` 前，用当前 Agent 提供的结构化用户问答能力询问是否进入 Tasks。
- 固定选项：`1. 进入 Tasks / 2. 跳过 Tasks / 3. 自由输入补充信息`。用户选 `1` 后创建或复用 Task List，并把 trick 文档写入索引。
- 若用户选 `2`，仍可正常沉淀 trick，但不创建 Task List。
