...
data QARequest = QARequest { featureKind : FeatureKind, changesSecurityBoundary : Bool, changesAuthorization : Bool, changesPersistentData : Bool, changesExternalIntegration : Bool, changesReleaseOrDistributionContract : Bool, changesRuntimeOrReviewerGovernance : Bool }
...
selectQARunner input q env run = qaRunnerGate (qaRunnerRequired q) (selectTaskAgent QA env) run (case input of ResumeQARunner approval -> Just approval; _ -> Nothing)

qaRunnerRequired :: QARequest -> Bool
qaRunnerRequired q = featureKind q /= NonFunctional || changesSecurityBoundary q || changesAuthorization q || changesPersistentData q || changesExternalIntegration q || changesReleaseOrDistributionContract q || changesRuntimeOrReviewerGovernance q
...
qaRunnerGate True (SelectionNeedsOwnerApproval reason) NotStarted _ = Blocked reason
...
QA 按上述 gate 决定本地执行或独立 runner。`qaRunnerRequired` 是唯一风险分类：功能性/mixed feature，以及安全、授权、持久化数据、外部集成、发布/分发契约、runtime 或 reviewer governance 变更，都必须使用独立 runner；该结果不能由 owner 或调用方覆盖。`ResumeQARunner` 必须由主入口 typed union 传入；`LaunchQARunner` 先写报告的 `runner_state: awaiting` 与 `runner_id`，`Awaiting (QARunner ref)` 只从该 id 恢复；高风险 runner 不可用、失败或 hard block 时分别持久化为 `blocked`、`runner-failed`、`blocked` 并填写 `runner_reason`，不得使用 owner approval 静默本地降级。runner 按 agent-conventions 的 plan / read-only 等价 mode 一步启动，但它是辅助执行者，不是 QA owner：
...
进入 QA 前必须读取：`.codestable/attention.md`、`{slug}-design.md`、`{slug}-checklist.yaml`、`{slug}-review.md`、goal/gate 模式下的 evidence pack/gate results/DoD results、实现完成汇报、`git status --short`、`git diff`、design 必跑验证命令、review Test And QA Focus/Residual Risk、项目测试入口、feature 性质判断。
