...
1. 如果 runtime 能显式启动 Cursor 自定义 subagent，优先使用 `codestable-code-reviewer`。不要调用 `codestable:code-reviewer` 冒号形式；它只是旧逻辑合约名，不是当前 Cursor 默认可执行名。该 custom subagent 应运行在当前对话主模型的最高思考档；agent 定义中的 `model:` 可作为 Cursor 宿主兜底（例如 `gpt-5.6-sol`），不改变「优先当前主模型最高档」的派发意图。若 runtime 实际落到 Explore/Fast 或其它低档异构模型，不得采信，退回步骤 2 的 model-safe 判定。
...
5. 无法固定目标模型最高档、也不能保证父模型继承时，必须由当前主模型最高思考档 self review，并写 `reviewer: self`。禁止把 actual model / `unknown` 仅记为 residual risk 后继续把 bridge 结果当独立 reviewer evidence。
