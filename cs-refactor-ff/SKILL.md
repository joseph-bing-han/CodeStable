---
name: cs-refactor-ff
description: refactor 流程的超轻量通道——直接识别 1-3 条低风险优化、一次确认、原地改、跑测试自证，完成后仍进入 `cs-code-review`。触发：用户说"快速重构"、"小重构"、"简单优化下 XX 函数"、"别那么多步骤"，且改动在单函数 / 单组件局部、有测试可自证。
---
...
- 等级：`auto`（重要变更）。本 skill 虽然跳过 scan/design/checklist，但会修改代码并可能写 refactor-note，动手前必须先创建或复用 Task List。
- 每完成一次低风险优化、验证或 note 更新后，先更新 Task List 的步骤状态和 CodeStable 文档索引，再继续原流程。
- fastforward 重构完成并验证通过后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；review 通过后将 Task List 标记 `completed` 并立即进入 `cs-task archive`；退出条件以 active 中无同名残留为准。

很轻：没有 scan 清单、没有 design doc、没有 checklist，改完一句话汇报就行。

## 实现门禁（Task + review evidence）

改前先确认三件事：本次优化满足 fastforward 入场 3 条、active Task 已登记本 refactor、验证命令能自证行为等价。任一项缺失就退回完整 `cs-refactor` 或补齐 Task，不开始改代码。

自证通过后不能只汇报“测试已过”。必须进入 `cs-code-review` 产出 `{slug}-code-review.md`；review 通过前 Task 不能 completed，也不能进入 commit。
...
- 完成后立即进入 `cs-code-review` 做独立 diff 评审；Critical/Important 未清零不进 commit，scoped-commit 发起权归 `cs-code-review`
- 跑测试（单元 / 集成 / 类型检查 / lint）
...
验证通过后自动进入 `cs-code-review`。Critical / Important 未清零前，不标记完成、不进入 commit。

review 通过后如需 scoped-commit，由 `cs-code-review` 统一按 `shared-conventions.md` 第 4 节发起确认并执行。

---

## 文件产出

动手阶段默认**不写 scan / design / checklist**。但因为最终 `cs-code-review` 是强制门禁，进入 review 前必须有一个最小落点：

- 创建 `.codestable/refactors/{YYYY-MM-DD}-{slug}/`
- 至少由 `cs-code-review` 写入 `{slug}-code-review.md`
- 用户明确说"这次要留个记录"时，再额外写 `{slug}-refactor-note.md`，内容就是上面那句汇报 + 一段"做了什么 / 为什么"

`{slug}` 与日期沿用标准 refactor 目录规则：日期取本次第一次落盘当天，slug 用本次重构目标的英文 kebab-case 概括，和后续 `cs-code-review` 写入的目录名保持一致。

也就是说，fastforward 的轻量体现在**不写标准三件套**，不是完全没有可追溯产物。
