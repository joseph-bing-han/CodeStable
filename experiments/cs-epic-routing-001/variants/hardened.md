...
`restoreEpicStage :: EpicState -> Intent -> EpicOutcome` 落在下方「状态机」表；子 design 阶段是连续 `ChildDesignBatch` loop（见「Child design batch loop」），在 `ConfirmAllChildDesign`（统一确认所有 design）之前不得 final answer。`HumanCheckpoint` 三点见下方「人工 checkpoint」。
