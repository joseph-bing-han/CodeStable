...
- 等级：`auto`（重要变更）。本 skill 在创建 issue 目录、写 `{slug}-report.md`，或判定快速通道后准备进入修复前，必须先创建或复用该 issue 的 Task List。
- 每次 report 草稿落盘、`severity` / 复现步骤更新、或路径判定（标准 / 快速）明确后，先更新 Task List 的步骤状态和 CodeStable 文档索引。
- report 阶段结束不标记 `completed`；标准路径把 Task 交给 `cs-issue-analyze`，快速通道则直接交给 `cs-issue-fix` 继续复用。
- **创建时机不可后移**：只要下一步会创建 issue 目录、写 report，或快速通道将要改代码，必须先自动进入 `cs-task create/recovery`。不得先改代码、最后补 Task。
- 快速通道即使省略 report / analysis，也必须创建 issue 目录和 Task List；Task 的 CodeStable 文档索引先登记计划中的 `{slug}-fix-note.md`，修完落盘后再更新为实际路径和验证状态。
- **chat-only report 不算完成**：只在对话里整理 issue 摘要、初步分析或下一步建议，不算 report 阶段完成。进入 `cs-issue-analyze` 前，必须已经有 report 文件和 active Task，且 Task `owner_skill=cs-issue-analyze`、`related_docs` 包含 report 路径。
...
3. **确定 issue 目录名和 Task 起点**——跟用户商定或按现象生成 slug，日期前缀用今天（环境信息 `currentDate`）。一旦准备落盘或进入快速修复，先创建 / 复用 `.codestable/tasks/active/{slug}.md`，再创建 issue 目录。快速通道也要建 issue 目录，`{slug}-fix-note.md` 放那
4. **快速通道判断（唯一正式判定点）**——按用户线索**读一下相关代码**（Grep / Read 定位）：
   - **能一眼确定根因**（能给出 `{文件}:{行号}`、修复改动小 1-2 处、无跨模块影响风险）→ 告诉用户"我已看到问题所在，可以走快速通道：直接告知根因和修复方案，你确认后我立刻修，修完你验证，只写一份 `{slug}-fix-note.md`"。同意后自动进入 `cs-issue-fix`（快速通道模式）
...
- [ ] active Task List 已创建 / 复用，`related_docs` 已登记 report，`owner_skill` 已切到下一阶段

---

## 退出后

用结构化 L2 checkpoint 展示 report 摘要并让用户选：`1. 通过并继续（推荐）` / `2. 修改 report` / `3. 暂停` / `4. 自由输入补充信息`。用户选 1 后，当前 agent 必须立刻读取并执行 `cs-issue-analyze`，不能回复“建议使用 `cs-issue-analyze`”或要求用户重新触发。

别在用户确认 report 前开始分析根因；但确认通过后不要再要求用户手动触发下一阶段。
