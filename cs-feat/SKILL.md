...
brainstorm 是讨论层独立入口，会分诊：case 1（清楚 → 直接 design）/ case 2（小需求继续讨论 → 落 brainstorm note）/ case 3（大需求 → 自动交接 `cs-roadmap`）。只有 case 2 在 feature 目录产出 brainstorm note。

本技能不替阶段技能写代码或文档；它负责判断当前 feature 走到哪步，并自动进入对应子技能。只有阶段 review、范围歧义或风险操作才停下来用结构化问题确认。

**这是 feature 的完整入口，不是半路由器**：用户从 `cs` 根入口被路由过来，和用户直接点名 `cs-feat`，后续闭环要求完全一致。只要当前阶段可确定，就继续沿同一条 feature 主线走到 `cs-code-review`、`cs-task archive` 和收尾建议，不让用户退回根入口重开，也不允许只“建议使用”下一阶段 skill。

## Task 接入

- 等级：`route-only`。本 skill 只判断 feature 当前阶段，不直接创建 Task List。
- 一旦自动编排到 `cs-feat-design` / `cs-feat-impl` / `cs-feat-accept` / `cs-feat-ff`，由对应下游 skill 在首次落盘前自动创建或复用 Task List，并按 `modify -> update -> accept/review -> archive` 闭环。
- **硬性守卫**：本 skill 不能因为需求很清楚就直接修改代码 / 配置 / 文档。目标阶段会写文件时，必须先交给对应下游 skill，并由下游在首次落盘前完成 Task List create / recovery。
- fastforward 也必须有 Task List；它只省 design / checklist / acceptance，不省任务账本、ff-note 和 code-review。
...
| 4 最终质量门禁 | `cs-code-review` | code-review.md | 独立 reviewer 审当前 diff |

阶段间有 gate 和人工 checkpoint。方案先过 design-review，再交给用户整体确认；用户没明确放行，下一阶段别开始——防止 AI 一口气从需求跑到代码、跑出来才发现走偏。

阶段 0 可选且是 feature 流程的**外部入口**——`cs-brainstorm` 同时服务 feature 和 roadmap。case 3（大需求）讨论自动交接给 `cs-roadmap` 不再回 feature 流程；roadmap 拆出子 feature 后从 `cs-feat-design` 的"从 roadmap 条目起头"入口进来。
...
**fastforward 底线**：直接实现不等于裸改。`cs-feat-ff` 启动后，首次修改代码或配置前必须先创建 / 复用 Task List；实现后必须写 `{slug}-ff-note.md` 并进入 `cs-code-review`。
...
- brainstorm：用户脑子里模糊，AI 问用户答。判 case 3 时自动交接 `cs-roadmap` 不回 feature；只有 case 2 产出 brainstorm note
