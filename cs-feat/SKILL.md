---
name: cs-feat
description: 新功能开发的子流程入口，把"加个 X 能力"从想法自动编排到验收与最终 code review。触发：用户说"做新功能"、"加个 X"、"实现 XX"。根据已有产物自动进入 brainstorm / design / fastforward / implement / acceptance / code-review。不处理 bug。
---
...
```
(想法模糊先去 cs-brainstorm 分诊) → 方案设计（名词层 + 编排层 + 验收契约 + 推进策略切片）→ 分步实现 → 验收闭环 → code review
```
...
- 一旦自动编排到 `cs-feat-design` / `cs-feat-impl` / `cs-feat-accept` / `cs-feat-ff`，由对应下游 skill 负责按 `auto` 策略创建或复用 Task List。
...
├── {slug}-ff-note.md          ← fastforward 回顾（仅 ff）
...
阶段间有 L2 review checkpoint。上一阶段没拿到用户明确放行，下一阶段别开始；但用户选择“通过并继续”后必须自动进入下一阶段，不要求重新输入 skill 名。
...
| `{slug}-ff-note.md` 已存在、准备 commit / PR / merge | `cs-code-review` |
| fastforward 代码已写完但 `ff-note` 还没落盘 | `cs-feat-ff` |
| acceptance 已完成、准备 commit / PR / merge | `cs-code-review` |
