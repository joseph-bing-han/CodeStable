...
- 等级：`auto`（次要变更但强制留痕）。本 skill 在首次写 `docs/dev/` 或 `docs/user/` 指南前，必须自动创建或复用 Task List。
- 每次 guide 落盘或索引更新后，先更新 Task List 的步骤状态和文档索引，再继续原流程。
- 用户确认指南内容可用后，将 Task 标记 `completed` 并立即进入 `cs-task archive`；不允许写完 guide 直接结束。
