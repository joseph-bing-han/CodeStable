...
实现完成门禁。用于 Stop hook 或手动检查：有实现代码变更时，已完成的 feature / issue / refactor 要有 `{slug}-review.md`，且默认必须声明 subagent reviewer。
...
```bash
# 汇报前确认没有遗漏的人审 / follow-up / lifecycle 阻塞
python3 .codestable/tools/codestable-doctor.py --root . --json
```
