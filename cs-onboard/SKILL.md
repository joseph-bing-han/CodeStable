...
- 等级：`bootstrap`。onboard 只搭骨架/归旧档，**自身不创建 Task List**——`tasks/` 目录建好即止，第一条 Task 由后续落盘的子技能（cs-feat-design / cs-issue-report 等）首次写产物时创建。
- 迁移路径中若改动已有文件，照常在汇报里逐条列出让 owner 拍板，不依赖 Task List 追踪。
...
- `.codestable/{requirements,roadmap,goals,features,issues,refactors,audits,tasks/active,tasks/archived,compound}/.gitkeep`
...
- [ ] `.codestable/` 各聚合根目录（requirements/architecture/roadmap/goals/features/issues/refactors/audits/tasks/compound）都存在
