...
- 等级：`auto`（重要变更）。本 skill 在创建 `.codestable/` 骨架、复制 reference/tools、写 attention 或迁移旧文档前，必须先创建或复用 Task List。
- 每次创建目录、释放模板、迁移文档或更新 onboarding 审计记录后，先更新 Task List 的步骤状态和 CodeStable 文档索引，再继续原流程。
- onboard / migration 落盘完成并用户确认结果可用后，将 Task List 标记 `completed` 并立即进入 `cs-task archive`；退出条件以 active 中无同名残留为准。
...
│   ├── codestable-doctor.py / validate-implementation-review.py / build-review-packet.py
│   └── codestable-spec-governance.py / plan-commits.py / codestable-backlog.py …
└── reference/                  跨子技能共享参考（onboard 整目录释放）
    ...
    └── spec-governance-tools.md / tools-context.md
```

> `tools/` 与 `reference/` 由 onboard 整目录复制，自动带上 review evidence、上下文包、commit planner 和共享口径，无需逐个列举维护。
...
- `.codestable/hooks/`（如技能包存在则复制；当前核心流程不依赖 hook）
...
test -d <技能包路径>/cs-onboard/hooks && cp -rf <技能包路径>/cs-onboard/hooks/. .codestable/hooks/ || true
...
if (Test-Path <技能包路径>\cs-onboard\hooks) { Copy-Item -Recurse -Force <技能包路径>\cs-onboard\hooks\* .codestable\hooks\ }
```
...
- [ ] `.codestable/tools/`、`.codestable/reference/` 已从技能包复制；`.codestable/hooks/` 如技能包存在则已同步
