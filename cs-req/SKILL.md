...
- 等级：`auto`（重要变更）。本 skill 在首次创建或更新 `requirements/{slug}.md`、`VISION.md` 或变更日志前，必须先创建或复用 Task List。
- `draft` / `backfill` / `update` 模式下，每次 req 文档落盘、`status` 变更、索引刷新或 `implemented_by` 调整后，先更新 Task List 的步骤状态和 CodeStable 文档索引。
- 当前 req 工作完成并用户 review 通过后，将 Task List 标记 `completed` 并立即进入 `cs-task archive`；退出条件以 active 中无同名残留为准。
