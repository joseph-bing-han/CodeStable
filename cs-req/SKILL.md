...
- 等级：`auto`。req 文档落盘动作复用或创建当前流程的 Task List（`cs-task`）。
- 写 / 刷新 requirement 产物前复用当前流程 Task List，无 Task 不动手；产物落盘先更新 Task List 再继续。
- req 多由 design / acceptance 流程驱动，非末端不标 `completed`；收口交对应主流程。
