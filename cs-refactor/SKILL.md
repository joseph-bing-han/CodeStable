---
name: cs-refactor
description: 代码优化的子流程入口，处理"行为不变、结构变"的工作（结构 / 性能 / 可读性），按 scan → design → apply → code-review 推进，并继续到 Task 归档。触发：用户说"优化一下 / 重构 / 重写 / 拆一下 / 性能不行 / 代码太长"且不夹带行为改动。不处理新需求 / bug / 跨模块架构重划。
---
...
**这是 refactor 的完整入口**：用户直接点名 `cs-refactor`，和先走 `cs` 再被路由过来没有区别；后续要沿 scan / design / apply / code review / archive 跑到底，而不是停在某个局部阶段建议；下一阶段明确时当前 agent 必须继续执行目标 skill。

## Task 接入

- 等级：`auto`（重要变更）。本 skill 在创建 refactor scan/design/checklist、修改代码或写 apply-notes 前，必须先创建或复用 Task List。
- 每次阶段落盘、步骤推进或验证结果更新后，先更新 Task List 的步骤状态和 CodeStable 文档索引，再继续原流程。
- refactor apply 完成并确认行为等价后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；review 通过后将 Task List 标记 `completed` 并立即进入 `cs-task archive`；退出条件以 active 中无同名残留为准。

```
 scan（扫优化点清单）→ design（和用户定做哪几条 + 顺序）→ apply（逐条执行，每步人工放行）→ code review
```

**核心纪律**：行为等价是底线。一旦会改外部可观察行为 → 不走 refactor，走 feature（需求变）或 issue（bug 修）。

## Apply 门禁（Task + review evidence）

进入 apply 前先确认三件事：refactor design 已 approved、checklist 存在且当前步骤可定位、active Task 已登记本 refactor。任一项缺失就退回对应阶段补齐，不开始改代码。

apply 完成后不能只汇报“测试已过”。必须先写 `{slug}-apply-notes.md`、更新 checklist 和 Task 账本，再立即进入 `cs-code-review` 产出 `{slug}-code-review.md`；review 通过前 Task 不能 completed。
...
| 4 code review | code-review.md | 独立 reviewer 审当前 diff |
...
- 确认通过后进入 `cs-code-review`；review 通过后再收尾 commit，message 引用 refactor 目录
