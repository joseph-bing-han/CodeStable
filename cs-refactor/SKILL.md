---
name: cs-refactor
description: 代码优化的子流程入口，处理"行为不变、结构变"的工作（结构 / 性能 / 可读性），按 scan → design → apply → code-review 推进。触发：用户说"优化一下 / 重构 / 重写 / 拆一下 / 性能不行 / 代码太长"且不夹带行为改动。不处理新需求 / bug / 跨模块架构重划。
---
...
- refactor apply 完成并确认行为等价后，自动把 Task List 的 `owner_skill` 切到 `cs-code-review`；review 通过后将 Task List 标记 `completed`，归档由 `cs-task` 执行并清理 active 残留。
