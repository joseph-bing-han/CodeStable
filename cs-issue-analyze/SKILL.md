...
- 等级：`auto`（重要变更）。本 skill 必须复用当前 issue 的 Task List；若 report 阶段尚未建 Task List，则此处补建后再继续分析。
- 每次 analysis 草稿落盘、根因定位更新、影响面补充、用户确认推荐方案后，先更新 Task List 的步骤状态、`owner_skill` 和 CodeStable 文档索引。
- 分析阶段结束不标记 `completed`；用户确认方案后，自动把下一步 owner skill 和已确认方案交给 `cs-issue-fix` 继续闭环。
- **无 Task / 无 report 不分析**：如果用户直接触发 analyze，但 report 文件或 active Task 缺失，先自动回 `cs-issue-report` 补齐并 backfill Task；不能把 chat 里的分析当成正式产物。
...
1. **问题报告和 Task 存在且已确认**——读 `{slug}-report.md` 与匹配的 `.codestable/tasks/active/*.md`，确认 report `doc_type=issue-report` 且 `status=confirmed`，Task `workflow=issue` 且 `related_docs` 包含 report。不完整 / 状态不对 / Task 缺失 → 回 `cs-issue-report` 补齐。`cs-issue-report` 已判走标准路径就按标准路径走，不二次改判
...
3. 用结构化选项让用户确认方案；用户明确确认后，当前 agent 自动进入阶段 3 `cs-issue-fix`，不能只推荐下一条命令
...
- [ ] active Task List 已更新，`related_docs` 已登记 analysis，`owner_skill` 已切到 `cs-issue-fix`

---

## 退出后

用结构化 L2 checkpoint 展示根因与推荐方案并让用户选：`1. 按推荐方案修（推荐）` / `2. 选择其它方案` / `3. 修改分析` / `4. 暂停或自由输入`。用户选择修复方案后，当前 agent 必须立刻读取并执行 `cs-issue-fix`，不能回复“建议使用 `cs-issue-fix`”或要求用户重新触发。

别在用户确认方案前改代码；但确认后不要再要求用户手动触发下一阶段。
...
- 只列一种修复方案——至少两种
- 分析完直接开始改代码——必须等用户确认；确认后应自动进入修复，不让用户复制粘贴下一条命令
