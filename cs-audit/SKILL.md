---
name: cs-audit
description: 系统审计——从代码中主动发现 bug 隐患、安全漏洞、性能问题、可维护性债务和架构偏离，产出批量发现清单；用户选定 finding 后自动接到 issue / refactor 主线。触发：用户说"审查系统"、"审计代码"、"扫描问题"、"找找 bug"、"有什么可以优化的"。
---
...
- 等级：`auto`（重要变更）。本 skill 在创建 audit 目录、写 `index.md` 或任一 `finding-*.md` 前，必须先创建或复用 Task List。
- 每次新增 finding、刷新 index、调整严重度或记录下一步建议后，先更新 Task List 的步骤状态和 CodeStable 文档索引，再继续原流程。
- audit 产物落盘并完成用户 review 后将 Task List 标记 `completed` 并立即进入 `cs-task archive`；退出条件以 active 中无同名残留为准。

本技能只发现、不定修。修是 `cs-issue` / `cs-refactor` 的事。

作为入口，audit 本身可以在发现清单这里收口；但只要用户从某条 finding 继续推进，handoff 不能停在“建议下一步”，而要自动读取并执行 `cs-issue` / `cs-refactor`，继续完整闭环。
...
用户选哪条 → 自动交给 `cs-issue` 或 `cs-refactor` 并沿下游主线继续。`cs-audit` 自己不修。
