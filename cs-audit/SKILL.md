...
- 等级：`auto`。审计自身是一条独立流程，对应自己的 Task List。
- 产物（`index.md` / `finding-*.md`）首次落盘前复用或创建当前流程的 Task List（`cs-task`），无 Task 不动手；落盘先更新 Task List 再继续。
- 审计只发现不修复，非末端不标 `completed`；修复交 `cs-issue` / `cs-refactor` 各自流程收口。
