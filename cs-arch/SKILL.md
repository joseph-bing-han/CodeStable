...
- 等级：`auto`（重要变更）。本 skill 在首次创建或更新 `.codestable/architecture/` 下文档、索引或检查报告前，必须先创建或复用 Task List。
- `backfill` / `update` / `check` 三种模式下，每次文档落盘、索引更新、分组搬迁或检查报告生成后，先更新 Task List 的步骤状态和 CodeStable 文档索引。
- 当前模式产物完成并用户 review / 确认通过后，将 Task List 标记 `completed`；归档由 `cs-task` 执行并清理 active 残留。
