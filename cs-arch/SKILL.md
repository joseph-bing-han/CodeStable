...
- 等级：`auto`。架构文档的 update / check / backfill 复用或创建当前流程的 Task List（`cs-task`）。
- 写 / 刷新 architecture 产物或落 check 报告前复用当前流程 Task List，无 Task 不动手；产物落盘先更新 Task List 再继续。
- arch 常由 feature / acceptance 流程驱动，非末端不标 `completed`；收口交对应主流程。
